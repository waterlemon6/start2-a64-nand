...
#define MAX_JS_CHAR_COUNT 15
#define JSM_RESET_COUNT 10
...
{
	DriveSignal(SIG_TMS, mode, TCK_TOGGLE);
...
int LoadJI(int action, int dev_count, int* ji_info)
{
	...
	for(i = 0; i < dev_count; i++)
	{
		...
		{
			//error=1;
			//printf("Error: JTAG chain broken!   Error: Bits unloaded: 0x%X\n", record);
			return error;
		}
	}
	/* Move JSM to UPDATE_IR */
...
int ReadTDO(int bit_count, int data, int inst)
{
	...
	unsigned int i, max = (unsigned int) (inst ? (bit_count - 1) : bit_count);
	//printf("%d %d %d \n", bit_count, data, inst);
	for(i = 0; i < max; i++)
	{		
		unsigned int mask = 1;
	    //read
		tdo = (unsigned int) (READ_TDO ? (1 << i) : 0);
		record = record | tdo;

		//write
        mask = mask << i;
        tdi = (data & mask) >> i;
        DriveSignal(SIG_TDI, tdi, TCK_TOGGLE);
	}
	return record;
}
